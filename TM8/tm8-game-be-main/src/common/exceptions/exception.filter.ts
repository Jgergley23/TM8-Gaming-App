...
import { TGMDException, TGMDInternalException } from './custom.exception';
...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  private mongoExceptionHandler(exception: any): IExceptionInfo {
    ...
    const handler =
      ...
      ((): TGMDException =>
        new TGMDInternalException('Something went wrong, please try again'));

    return this.TGMDErrorHandler(handler());
  }

  /**
   * Handles TGMDError exceptions.
   */
  private TGMDErrorHandler(exception: TGMDException): IExceptionInfo {
   ...
   */
    ...
    if (exception instanceof TGMDException) {
      data = this.TGMDErrorHandler(exception);
    }
