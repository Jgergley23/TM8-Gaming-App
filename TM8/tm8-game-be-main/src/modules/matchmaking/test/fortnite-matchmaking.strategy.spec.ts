...
import { FortniteMatchmakingStrategy } from '../services/fortnite-matchmaking.strategy';
import {
...
describe('FortniteMatchmakingStrategy', () => {
  let fortniteMatchmakingStrategy: FortniteMatchmakingStrategy;

  let matchmakingPreferenceFilteringService: jest.Mocked<AbstractMatchmakingPreferenceFilteringService>;

  beforeEach(() => {
    const { unit, unitRef } = TestBed.create(
      FortniteMatchmakingStrategy,
    ).compile();

    fortniteMatchmakingStrategy = unit;
  ...
  describe('filterPreferences', () => {
    it('should return array of ids on success', async () => {
      ...
      //act
      const result = fortniteMatchmakingStrategy.filterPreferences(
        currentUserGameDataMock,
        usersGameDataMock,
      );
