...
part 'google_sign_up_cubit.freezed.dart';

@injectable
class GoogleSignUpCubit extends Cubit<GoogleSignUpState> {
  GoogleSignUpCubit() : super(const GoogleSignUpState.initial());

  Future<void> googleSingUp({required VerifyGoogleIdInput body}) async {
    try {
      emit(const GoogleSignUpState.loading());

      final result = await api.apiV1AuthGoogleVerifyPost(body: body);
      ...
      emit(GoogleSignUpState.loaded(response: result.bodyOrThrow));
    } catch (e) {
      logError(e.toString());
      emit(GoogleSignUpState.error(error: e.toString()));
    }
  }
}
